...
c.execute('''CREATE TABLE IF NOT EXISTS expenses)
             (id INTIGER PRIMARY KEY, user_id INTIGER, description TEXT, amount REAL, date TEXT)''')
conn.commit()
...
def add_expense():
    ...
    c.execute( _sql: "INSERT INTO expenses (user_id, description, amount, date) VALUES (?, ?, ?, ?)",
              _parameters: (user_id, description, amount, date))
    conn.commit()

    messagebox.showinfo(title: "success", message: "expense added successfully")
    ...
    def split_bill()
        ...
        share _amount = total_amount / num_people

        messagebox.showinfo( title: "bill split", message: f"each person owes {share_amount:.2f}.")

    1 usage
    def get_debts():
        user_id = int(user_id_debts_entry.get())
        c.execute( _sql: "SELECT user_id, SUM(amount) FROM expenses WHERE user_id != ? GROUP BY user_id", _parameters: (user_id,))
        debts = c.fetchall()
        ...
        messagebox.showinfo( title: "debts", message)
...
add_expence_button = tk.Button(add_expenese_frame, text="Add Expense", command=add_expense)
...
total_amount_label = tk.Entry(split_bill_frame)
total_amount_entry.grid(row=0, column=1)

num_people_ label = tk.Label(split_bill_frame, text="Number of people:")
num _people_label.grid(row=1, column=0)
